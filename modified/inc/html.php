 ...
 * 
 * This code has been edited and not is original dokuwiki code.
 */
...
/* A code where it generates a scrollspy-compatible TOC. Made with claude.ai. */
function _html_buildlist($data, $class, $func, $lifunc = null, $forcewrapper = false) {
    if ($data === []) {
        return '';
    }
    ...
    // Modified callback function to use Bootstrap nav-item class
    if (!is_callable($lifunc)) {
        $lifunc = static fn($item) => '<li class="nav-item">';
    }

    foreach ($data as $item) {
        if ($item['level'] > $level) {
            // Open new list with Bootstrap classes
            for ($i = 0; $i < ($item['level'] - $level); $i++) {
                if ($i) $html .= '<li class="nav-item">';
                $html .= "\n" . '<ul class="nav flex-column">' . "\n";
                $html .= '<nav>' . "\n"; // Add nav wrapper for nested levels
                $open++;
            }
            $level = $item['level'];
        } elseif ($item['level'] < $level) {
            ...
            $html .= '</li>' . "\n";
            while ($level > $item['level'] && $open > 0) {
                // Close higher lists with nav wrapper
                $html .= '</nav>' . "\n"; // Close nav wrapper
                $html .= '</ul>' . "\n" . '</li>' . "\n";
                $level--;
                $open--;
    ...
    $html .= '</li>' . "\n";
    while ($open-- > 0) {
        $html .= '</nav></ul></li>' . "\n"; // Include nav closing tag
    }

    if ($forcewrapper || $start_level < 2) {
        // Add Bootstrap navbar wrapper
        $html = "\n" . '<nav id="dw__toc" class="navbar sticky-top">' . "\n" .
               '<ul class="nav flex-column">' . "\n" . 
               $html . 
               '</ul>' . "\n" .
               '</nav>' . "\n";
...
function html_TOC($toc)
{
    ...
    $out .= _html_buildlist($toc, 'toc', 'html_list_toc', null, true);
    $out .= '<!-- TOC END -->' . DOKU_LF;
    return $out;
}
 ...
 * @return string html
 */
// Modified TOC link function to use Bootstrap classes
function html_list_toc($item) {
    ...
    // Add Bootstrap nav-link class and make first item active
    static $first = true;
    $activeClass = $first ? ' active' : '';
    $first = false;

    return '<a class="nav-link' . $activeClass . '" href="' . $link . '">' . hsc($item['title']) . '</a>';
}
